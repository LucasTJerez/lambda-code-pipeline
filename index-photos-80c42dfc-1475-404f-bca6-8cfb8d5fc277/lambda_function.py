import logging
import base64
import json
import boto3
import os
import time
import requests
s3 = boto3.client('s3')
def lambda_handler(event, context):
   
    print(event)
    s3_info = event['Records'][0]['s3']
    bucket_name = s3_info['bucket']['name']
    key_name = s3_info['object']['key']
    print(bucket_name)
    print(key_name)
    response = s3.head_object(Bucket=bucket_name, Key=key_name)
    print(response)
    # print(response['ResponseMetadata']['HTTPHeaders']['x-amz-meta-customlabels'])
    labels = []
    if 'x-amz-meta-customlabels' in response['ResponseMetadata']['HTTPHeaders']:
        labels = response['ResponseMetadata']['HTTPHeaders']['x-amz-meta-customlabels'].split(',')
    # print("change")
    
    # https://photosbucket02.s3.amazonaws.com/rename.jpg

    client = boto3.client('rekognition')
    ans = requests.get('https://' + bucket_name +'.s3.amazonaws.com/'+ key_name)

    # ans.text = ""
    
    # with open('https://' + bucket_name +'.s3.amazonaws.com/'+ key_name, "rb") as cf:
    tmp2=base64.b64decode(ans.text)
    # pass_object = {'S3Object':{'Bucket':bucket_name,'Name':base64_image}}
    
    
    resp = client.detect_labels(Image={'Bytes':tmp2})

    print("rekognition response")
    print(resp)
    timestamp = time.time()
    
    
    for i in range(len(resp['Labels'])):
        # if i == 4:
        #     break
        labels.append(resp['Labels'][i]['Name'])
    print('labels:')
    print(labels)
    
    fmt = {'objectKey':key_name,'bucket':bucket_name,'createdTimestamp':timestamp,'labels':labels}
    
    opensearchURL = "https://search-photos-uxu6px6tgt4pciyej5dr73yidi.us-east-1.es.amazonaws.com/photos/_doc"
    headers = {"Content-Type": "application/json"}
    
    response = requests.post(opensearchURL, headers= headers, auth=("master","Cloudcomputing!1"), data=json.dumps(fmt))
    
    print(response.text)
    
    return {
        'statusCode': 200,
        'body': json.dumps('Hello from Lambda!')
    }
